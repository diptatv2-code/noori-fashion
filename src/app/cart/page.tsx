  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8">
      <div className="flex items-center gap-2 text-sm text-dark-400 mb-6">
        <Link href="/" className="hover:text-brand">Home</Link><span>/</span><span className="text-dark-600">Shopping Cart</span>
      </div>
          ...
          {items.map((item, idx) => (
            <div key={`${item.product.id}-${item.selectedSize}-${idx}`} className="flex gap-4 p-4 border border-dark-100">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-brand font-semibold mt-2">{formatPrice(item.product.price)}</p>
