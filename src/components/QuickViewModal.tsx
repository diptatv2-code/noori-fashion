  ...
  return (
    <div className="fixed inset-0 z-[80] flex items-center justify-center p-4">
      ...
      <div className="relative bg-white max-w-2xl w-full max-h-[90vh] overflow-y-auto animate-slide-up shadow-2xl">
        <button onClick={() => setQuickView(null)} className="absolute top-3 right-3 z-10 p-1.5 bg-white/80 hover:bg-white shadow rounded-full">
        ...
        <div className="grid md:grid-cols-2">
          ...
          <div className="p-6">
            ...
            {product.fabric_type && (
              <p className="text-xs text-dark-400 mb-4">Fabric: {product.fabric_type}</p>
            )}
                ...
                <div className="flex flex-wrap gap-2">
                  {sizes.map((s) => (
                    <button
                      ...
                      className={`min-w-[40px] h-9 px-3 border text-sm transition-colors ${
                        selectedSize === s
                          ? "border-brand bg-brand text-white"
                          : "border-dark-200 hover:border-brand"
                      }`}
                    >
                      {s}
                    </button>
