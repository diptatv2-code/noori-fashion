    ...
    <div className="fixed inset-0 z-[70]">
      ...
      <div className="absolute right-0 top-0 h-full w-full max-w-md bg-white shadow-2xl animate-slide-in-right flex flex-col">
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b">
          <h2 className="font-display text-lg font-semibold">শপিং ব্যাগ ({items.length})</h2>
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4">
          {items.length === 0 ? (
            <div className="text-center py-16">
              ...
              <p className="text-dark-400 font-bengali">আপনার ব্যাগ খালি</p>
              <button onClick={() => setCartOpen(false)} className="mt-4 btn-primary text-sm">শপিং শুরু করুন</button>
            </div>
          ) : (
              ...
              {items.map((item, idx) => (
                <div key={`${item.product.id}-${item.selectedSize}-${idx}`} className="flex gap-3 pb-4 border-b border-dark-100">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {item.selectedSize && <p className="text-xs text-dark-400 mt-0.5">সাইজ: {item.selectedSize}</p>}
        ...
        {items.length > 0 && (
          <div className="border-t px-5 py-4 space-y-3">
            ...
            <p className="text-xs text-dark-400">শিপিং চেকআউটে যোগ হবে</p>
