...
export default function CheckoutPage() {
  ...
  const [form, setForm] = useState({
    name: "", phone: "", email: user?.email || "", address: "", city: "Dhaka", area: "", payment: "cod" as "cod" | "bkash" | "nagad", txnId: "", notes: "",
  });

  const subtotal = getTotal();
  const shipping = subtotal >= 5000 ? 0 : form.city === "Dhaka" ? 80 : 150;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if ((form.payment === "bkash" || form.payment === "nagad") && !form.txnId) { setError("Please enter the transaction ID"); return; }
    ...
    try {
      const res = await fetch("/api/orders", {
        ...
        body: JSON.stringify({
          ...
          payment_method: form.payment, transaction_id: form.txnId || null,
            ...
            <div className="border border-dark-100 p-5">
              ...
              <div className="space-y-3">
                {[
                  ...
                  { value: "bkash", label: "bKash", desc: "Send to bKash number: 01701019541" },
                  { value: "nagad", label: "Nagad", desc: "Send to Nagad number: 01515653291" },
                ].map((pm) => (
              ...
              {(form.payment === "bkash" || form.payment === "nagad") && (
                <div className="mt-4 p-3 bg-yellow-50 border border-yellow-200">
                  <p className="text-xs text-yellow-800 mb-2">
                    Send {formatPrice(total)} to {form.payment === "bkash" ? "01701019541" : "01515653291"} and enter the transaction ID below.
                  </p>
