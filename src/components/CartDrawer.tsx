...
export default function CartDrawer() {
  const { items, isCartOpen, setCartOpen, removeItem, updateQuantity, getTotal } =
    useCartStore();
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4">
          {items.length === 0 ? (
            <div className="text-center py-16">
              ...
              <button onClick={() => setCartOpen(false)} className="mt-4 btn-primary text-sm">
                Start Shopping
              </button>
            </div>
          ) : (
              ...
              {items.map((item, idx) => (
                <div key={`${item.product.id}-${item.selectedSize}-${idx}`} className="flex gap-3 pb-4 border-b border-dark-100">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {item.selectedSize && (
                      <p className="text-xs text-dark-400 mt-0.5">Size: {item.selectedSize}</p>
                    )}
